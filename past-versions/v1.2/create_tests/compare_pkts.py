...
layers = ["ethernet", "vlan_0x8100_0", "custom_int_shim", "custom_int_layers", "vlan_0x88a8_0", "ipv4", "ipv6", "udp", "tcp", "payload"]
...
def print_packet(packet_data):
    data =[]
    #check the one that is not none
    if packet_data["ethernet"] is not None:
        data.append(f"eth({hex(packet_data['ethernet'].type)})")
    if packet_data["vlan_0x8100_0"] is not None:
        data.append(f"u-vlan({packet_data['vlan_0x8100_0'].vlan},{hex(packet_data['vlan_0x8100_0'].type)})")
    if packet_data["vlan_0x88a8_0"] is not None:
        data.insert(1, f"s-vlan({packet_data['vlan_0x88a8_0'].vlan},{hex(packet_data['vlan_0x88a8_0'].type)})")
    if packet_data["custom_int_shim"] is not None:
        data.insert(2, f"int_shim({packet_data['custom_int_shim'].int_count},{hex(packet_data['custom_int_shim'].next_hdr)})")
    if packet_data["custom_int_layers"]:
        #iterate in reverse order
        for i in range(len(packet_data["custom_int_layers"])-1,-1,-1):
            data.insert(3, f"int{len(packet_data['custom_int_layers'])-i}")
    if packet_data["ipv4"] is not None:
        data.append("payload")
    # print(data)
...
def parse_packet(pkt):
    """Parse a packet into its layers."""
    packet_data = {
        "ethernet": None,
        "vlan_0x8100_0": None,
        "custom_int_shim": None,
        "custom_int_layers": [],
        "vlan_0x88a8_0": None,
        "ipv4": None,
        "ipv6": None,
...
def parse_vlan_layer(pkt, offset, packet_data):
    ...
    offset += 4
    if vlan_layer.type == 0x0601:  # CustomINTShim
        parse_custom_int_shim(pkt, offset, packet_data)
    elif vlan_layer.type == 0x0800:
        parse_ip_layer(pkt, offset, packet_data)
...
def parse_custom_int_shim(pkt, offset, packet_data):
    shim_bytes = bytes(pkt)[offset:offset + 3]
    shim_layer = CustomINTShim(shim_bytes)
    packet_data["custom_int_shim"] = shim_layer

    offset += 3
    # Parse each CustomINT layer
    for _ in range(shim_layer.int_count):
        parse_custom_int(pkt, offset, packet_data)
        offset += 4  # Move to the next CustomINT layer

    # Evaluate the `next_hdr` field to determine the next layer
    if shim_layer.next_hdr == 0x0800:  # IPv4
        parse_ip_layer(pkt, offset, packet_data)
    elif shim_layer.next_hdr == 0x86dd:  # IPv6
        parse_ipv6_layer(pkt, offset, packet_data)
    elif shim_layer.next_hdr == 0x0806:  # ARP
        parse_arp_layer(pkt, offset, packet_data)
    elif shim_layer.next_hdr == 0x8100:  # VLAN (802.1Q)
        parse_vlan_layer(pkt, offset, packet_data)
    elif shim_layer.next_hdr == 0x88a8:  # QinQ VLAN (802.1ad)
        parse_qinq_vlan_layer(pkt, offset, packet_data)
    else:
        # Unknown or unsupported protocol
        raw_data = bytes(pkt)[offset:]
        packet_data["payload"] = Raw(raw_data)

def parse_custom_int(pkt, offset, packet_data):
    custom_int_bytes = bytes(pkt)[offset:offset+4]
    custom_int_layer = CustomINT(custom_int_bytes)
    packet_data["custom_int_layers"].append(custom_int_layer)
...
def compare_pkts(pkt1, pkt2, counter,print_output=True):
    global layers
    same_layers = []
    different_layers = []
    missing_layers = []
    ...
    return counter,same_layers,different_layers
...
def compare_packets(i):
    ...
    for pkt1, pkt2 in zip(packets[::2], packets[1::2]):
        ...
        counter,same_layers,different_layers =compare_pkts(packet_data_1, packet_data_2, counter,True)
        print("\nComparison Summary:")
        print(f"Same layers: {same_layers}")
        print(f"Different layers: {different_layers}")
