...
#===============================================================================
def main():
    p4f.print_console("blue","Case 48")
    ...
    # NO
    # Next Hop is not polka
    stg11_eg_table_name = 'pipe.Egress.eg_polka_dst_ep_tbl'
    stg11_eg_actions = ['export_int','rm_polka_int','add_s_vlan']

    #===================Add entries=====================
    #------Next Hop is not polka compatible------
    stg11_eg_table = bfrt_info.table_get(stg11_eg_table_name)
    stg11_eg_table.entry_del(dev_tgt,[])

    # key = stg11_eg_table.make_key([gc.KeyTuple("eg_intr_md.egress_port", egress_port)])
    key = stg11_eg_table.make_key([gc.KeyTuple("eg_intr_md.egress_port", egress_port),
                             ])
    data = stg11_eg_table.make_data([
        gc.DataTuple('new_vid', 900),
    ],action_name=f"Egress.{stg11_eg_actions[2]}")
    stg11_eg_table.entry_add(dev_tgt, [key], [data])    


    #------For the mirror pkt------    
    # key = stg11_eg_table.make_key([gc.KeyTuple("eg_intr_md.egress_port", egress_port)])
    key = stg11_eg_table.make_key([gc.KeyTuple("eg_intr_md.egress_port", mirror_port)])
    data = stg11_eg_table.make_data([],action_name=f"Egress.{stg11_eg_actions[0]}")
    stg11_eg_table.entry_add(dev_tgt, [key], [data])    
