...
#===============================================================================
def main():
    p4f.print_console("blue","Case 40")
    ...
    # YES
    stg_3_ig_topology_discovery_tbl_name = 'Ingress.ig_topology_discovery_tbl'
    stg_3_ig_topology_discovery_actions = ['topology_discovery']
    #===================Add entries=====================
    #---------------------------------------------------
    p4f.print_console("grey","Adding entries to tables",100,'-')
    # Add entries to the tables
    stg_3_ig_topology_discovery_tbl = bfrt_info.table_get(stg_3_ig_topology_discovery_tbl_name)
    # Add entries to the tables
    stg_3_ig_topology_discovery_tbl.entry_del(dev_tgt,[])
    key = stg_3_ig_topology_discovery_tbl.make_key([gc.KeyTuple("ig_intr_md.ingress_port", ingress_port),
                                                    gc.KeyTuple("hdr.ethernet.ether_type", 0x88CC)
                                                    ])
    data = stg_3_ig_topology_discovery_tbl.make_data([],action_name=f"Ingress.{stg_3_ig_topology_discovery_actions[0]}")
    stg_3_ig_topology_discovery_tbl.entry_add(dev_tgt, [key], [data])
