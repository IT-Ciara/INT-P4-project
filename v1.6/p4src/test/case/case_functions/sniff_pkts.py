...
def parse_pkts(pkt, binary_str):
    ...
    result.append(f"eth(0x{ether.type:04X})")
    ...
    if next_hdr == "0x0601":
        ...
        result.append(f"int_shim({custom_int_shim.int_count},{next_hdr})")
    ...
    if next_hdr == "0x8100":  # VLAN
        ...
        result.append(f"o-vlan({dot1q.vlan},{next_hdr})")
...
def print_pkt_summary(pkt, binary_str):
    result = []
    #Give me the pkt length
    pkt_len = len(pkt)
    # print("Packet Length: ", pkt_len)
    ...
    if next_hdr == "0x88A8":
        dot1ad = Dot1AD(payload)
        next_hdr = f"0x{dot1ad.type:04X}"
        result.append(f"s-vlan({dot1ad.vlan},0x{dot1ad.type:04X})")
        payload = bytes(dot1ad.payload)
    ...
    if next_hdr == "0x0601":
        ...
        next_hdr = f"0x{custom_int_shim.next_hdr:04X}"
        int_count = custom_int_shim.int_count
        result.append(f"int_shim({custom_int_shim.int_count},0x{custom_int_shim.next_hdr:04X})")
        #Print the size left of the received packet
        payload = bytes(custom_int_shim.payload)
...
def pkt_hex(pkt,create_pkt=False):
    ...
    # print_pkt_summary(pkt,binary_str)
