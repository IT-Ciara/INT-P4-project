...
def build_pkt(config):
    ...
    if next_hdr == 0x88A8:
        pkt /= Dot1AD(vlan=config["s_vlan"]["vid"], type=config["s_vlan"]["ether_type"])
        next_hdr = config["s_vlan"]["ether_type"]
    ...
    if config["u_vlan"]["valid"] and next_hdr == 0x8100:
        pkt /= Dot1Q(vlan=config["u_vlan"]["vid"], type=config["u_vlan"]["ether_type"])
        next_hdr = config["u_vlan"]["ether_type"]
...
def send_pkts(user_overrides=user_overrides,show_layer=None,interface=0):
    ...
    sendp(pkt, iface=f"{interface}", verbose=False)
