...
def main():
    ...
    data =ig_port_info_tbl.make_data([gc.DataTuple("user_port",ig_user_port),
                                      gc.DataTuple("core_port",ig_core_port),
                                      gc.DataTuple("transit_port",ig_transit_port),
                                        gc.DataTuple("edge_port",ig_edge_port)
                                        ],
                                        action_name=f"Ingress.{ig_port_info_tbl_actions[0]}")
    ...
    data=eg_edge_port_tbl.make_data([gc.DataTuple("user_port",eg_user_port),
                                      ...
                                      action_name=f"Egress.{eg_port_info_tbl_actions[0]}")
    ...
    if eg_user_port == 1:
        ...
        action =eg_user_port_tbl_actions[0]
        if action in new_vid_actions:
            new_vid = pkt_override['u_vlan']['vid']
            if 'modify' in action:
                new_vid+=50

        if action != 'nothing':
            #Add entries to the table
            key=eg_user_port_tbl.make_key([gc.KeyTuple("hdr.ethernet.ether_type",pkt_override['ethernet']['ether_type']),
                                        gc.KeyTuple("hdr.u_vlan.vid",u_vid,u_vid_mask),
                                        gc.KeyTuple("hdr.s_vlan.vid",s_vid,s_vid_mask),
                                        ])
            if new_vid != 0:
        ...
        action =eg_p4_sw_port_tbl_actions[0]

        #Add entries to the table
        key=eg_p4_sw_port_tbl.make_key([gc.KeyTuple("hdr.ethernet.ether_type",pkt_override['ethernet']['ether_type']),
        ...
        action =eg_transit_port_tbl_actions[0]
        #Add entries to the table
        key=eg_transit_port_tbl.make_key([gc.KeyTuple("hdr.ethernet.ether_type",pkt_override['ethernet']['ether_type']),
                                    gc.KeyTuple("meta.transit_port",eg_transit_port),
                                    ])
        data =eg_transit_port_tbl.make_data([gc.DataTuple("new_vid",pkt_override['s_vlan']['vid'])],
                                        action_name=f"Egress.{action}")
