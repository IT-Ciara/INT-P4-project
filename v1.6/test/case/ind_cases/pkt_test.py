...
#===============================================================================
def main():
    p4f.print_console("blue","Case Test")
    ...
    # ===== Stage: U-VLANS ========
    vlan_eg_table_name = 'pipe.Egress.eg_vlan_tbl'
    vlan_eg_table_actions = ['add_u_vlan','modify_u_vlan']
    ...
    vlan_eg_table = bfrt_info.table_get(vlan_eg_table_name)
    vlan_eg_table.entry_del(dev_tgt,[])

    key=vlan_eg_table.make_key([gc.KeyTuple("hdr.ethernet.ether_type",pkt_override['ethernet']['ether_type']),
                            # gc.KeyTuple("hdr.u_vlan.vid", int(pkt_override['u_vlan']['vid']),0xFF0),
                            gc.KeyTuple("hdr.u_vlan.vid", 0,0x0),
                            gc.KeyTuple("hdr.s_vlan.vid", 0,0x0),                                
                            ])

    data =vlan_eg_table.make_data([gc.DataTuple("new_vid",200)
                            ],
                            action_name=f"Egress.{vlan_eg_table_actions[0]}")
    vlan_eg_table.entry_add(dev_tgt, [key], [data])
