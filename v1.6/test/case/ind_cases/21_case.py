...
#===============================================================================
def main():
    p4f.print_console("blue","Case 21")
    ...
    ports_config = {
        "1": {
            ...
            "action": "add_s_vlan_rm_int_polka"
        }
    }
    ...
    # NO
    ig_stg_11_polka_tbl_name = 'pipe.Ingress.ig_output_polka_port_tbl'
    ig_stg_11_polka_tbl_actions = ['set_md']

    #===================Add entries=====================
    #---------------------------------------------------
    #Add entries to the table
    ig_stg_11_polka_tbl = bfrt_info.table_get(ig_stg_11_polka_tbl_name)
    ig_stg_11_polka_tbl.entry_del(dev_tgt,[])

    #Add entries to the table
    key = ig_stg_11_polka_tbl.make_key([gc.KeyTuple("ig_tm_md.ucast_egress_port",ports_data['egress_port'])])     

    data = ig_stg_11_polka_tbl.make_data([gc.DataTuple("egress_port",ports_data['egress_port']),
                                        gc.DataTuple("ing_mir",ing_mir),
                                        gc.DataTuple("ing_ses",ing_ses),
                                        gc.DataTuple("egr_mir",egr_mir),
                                        gc.DataTuple("egr_ses",egr_ses),
                                           ],action_name=f"Ingress.{ig_stg_11_polka_tbl_actions[0]}")
    ig_stg_11_polka_tbl.entry_add(dev_tgt, [key], [data])
