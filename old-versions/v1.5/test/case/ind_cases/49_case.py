...
#===============================================================================
def main():
    p4f.print_console("blue","Case 49")
    ...
    stg_4_ig_actions = ['set_user_port','rm_s_vlan']
    ...
    key = stg_4_ig_table.make_key([gc.KeyTuple("ig_intr_md.ingress_port", ingress_port),
                                   ...
                                   gc.KeyTuple("hdr.s_vlan.vid", pkt_override['s_vlan']['vid']),
                                   ])
    data = stg_4_ig_table.make_data([
        gc.DataTuple("egress_port", egress_port),
    ],action_name=f"Ingress.{stg_4_ig_actions[1]}")
    ...
    key = ig_table.make_key([gc.KeyTuple("ig_intr_md.ingress_port", ingress_port),
                             gc.KeyTuple("hdr.ethernet.ether_type",pkt_override['ethernet']['ether_type']),
                             gc.KeyTuple("hdr.u_vlan.vid", 0,0),
                            #  gc.KeyTuple("hdr.u_vlan.vid", int(pkt_override['u_vlan']['vid']),0xFF)
                            gc.KeyTuple("hdr.s_vlan.vid", 0,0x0),
                             ])
    data = ig_table.make_data([gc.DataTuple("egress_port", egress_port),
                               gc.DataTuple("endpoint", endpoint),
                            #    gc.DataTuple("new_vid",int(pkt_override['u_vlan']['vid'])+150),
                               ],
                              action_name=f"Ingress.{ig_actions[0]}") #Forward
    ig_table.entry_add(dev_tgt, [key], [data])

    # ===== Egress ======
    eg_table_name = "Egress.eg_no_polka_dst_ep_tbl"
    actions = ['add_int_polka']

    #===================Add entries=====================
    eg_table = bfrt_info.table_get(eg_table_name)
    eg_table.entry_del(dev_tgt,[])
