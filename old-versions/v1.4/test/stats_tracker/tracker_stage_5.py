...
def print_direct_counter_entries(entries):
    if not entries:
        ...
        for key, data in entries:
            vlan_id = key.get('hdr.outer_vlan.vid', {}).get('value', 'N/A')
            ...
            if vlan_id == 'N/A':
                print(f"Port: {ingress_port} | Packets: {packets:<8} | Bytes: {bytes_count:<8}")
            else:
                print(f"Port: {ingress_port} | VLAN ID: {vlan_id} | Packets: {packets:<8} | Bytes: {bytes_count:<8}")
...
def main():
    ...
    table = 'pipe.Ingress.ig_stg_5.ig_port_loop_tbl'
    print(f"\nTable: {table}")
    entries = get_all_entries(table, dev_tgt, bfrt_info)
    print_entries(entries, table, True)

    table = 'pipe.Ingress.ig_stg_5.ig_vlan_loop_tbl'
    print(f"\nTable: {table}")
    entries = get_all_entries(table, dev_tgt, bfrt_info)
    print_entries(entries, table, True)


    table = 'pipe.Ingress.ig_stg_5.ig_loop_counter'
    print(f"\nTable: {table}")
    entries = get_all_entries(table, dev_tgt, bfrt_info)
