...
from entries_generator import entries_stage_7
from scapy_pkts import pkts_stage_7
...
def start_packet_capture():
    ...
    cmd = f"tshark {interface_args} -w {pcap_file} -a duration:8 &"
...
def track_stats_and_send_packets():
    ...
    pkts_stage_7.create_send_pkt(8,0x8842)
    # time.sleep(10)  # Allow time for packet capture
    # count_pkts()  # Stop capture and count packets

    # start_packet_capture()  # Start tshark before sending packets
    # time.sleep(3)  # Allow capture process to initialize
    # pkts_stage_7.create_send_pkt(6,1000,6,5000)
    # time.sleep(10)  # Allow time for packet capture 
    # count_pkts()  # Stop capture and count packets

def main():
    p4f.print_main("Stage 7 Tests")
    ...
    if args.all:
        p4f.add_entries(entries_stage_7)
