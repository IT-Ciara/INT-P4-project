...
def main():
    ...
    data = edge_node_table.make_data([gc.DataTuple('Ingress.core_node.f1', 1)])
    ...
    key = edge_node_table.make_key([gc.KeyTuple('$REGISTER_INDEX', 0)])
    data = edge_node_table.make_data([gc.DataTuple('Egress.edge_node.f1', 0)])
    edge_node_table.entry_mod(dev_tgt, [key], [data])
    print("Register updated successfully.")
    ...
    # #===================Add entries=====================
    # #------For the mirror pkt------
    # eg_table = bfrt_info.table_get(stg11_eg_table_name)
    # eg_table.entry_del(dev_tgt,[])
    # # key = eg_table.make_key([gc.KeyTuple("eg_intr_md.egress_port", egress_port)])
    # key = eg_table.make_key([gc.KeyTuple("eg_intr_md.egress_port", mirror_port)])
    # data = eg_table.make_data([],action_name=f"Egress.{stg11_eg_actions[0]}")
    ...
    # mirror_table_name = '$mirror.cfg'
    # mirror_cfg_table = bfrt_info.table_get(mirror_table_name)
    # mirror_cfg_table.entry_del(dev_tgt,[])
    # key = mirror_cfg_table.make_key([gc.KeyTuple("$sid", ig_mirror_ses)])
