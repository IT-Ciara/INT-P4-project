...
def main():
    ...
    # ====== Stage 5: SDN Trace? ======
    # YES
    stg_5_ig_tbl_name = "Ingress.ig_sdn_trace_tbl"
    stg_5_ig_actions = ['send_to_cpu']
    #===================Add entries=====================
    #---------------------------------------------------
    src_eth = pkt_override['ethernet']['src_addr']
    src_eth_int = int(src_eth.replace(':', ''), 16)
    stg_5_ig_tbl = bfrt_info.table_get(stg_5_ig_tbl_name)
    stg_5_ig_tbl.entry_del(dev_tgt,[])
    key = stg_5_ig_tbl.make_key([gc.KeyTuple("hdr.ethernet.src_addr", src_eth_int)])
    data = stg_5_ig_tbl.make_data([],action_name=f"Ingress.{stg_5_ig_actions[0]}")
    stg_5_ig_tbl.entry_add(dev_tgt, [key], [data])
